...
| `components/MiniComparator/ComparisonTable.test.jsx` - viewport class group | 3 | Delete in #25 | Exact utility classes duplicated the viewport-cap suite; Chromium now covers the layout risk. |
| `components/MiniComparator/ComparisonTable.test.jsx` - empty state and variant marker groups | 3 | Simplify in #25 | Keep the empty-state and variant text contracts. Do not assert `border-l` or other utility classes. |
...
| `components/MiniComparator/FilterPanel.test.jsx` - CSS and legacy-position groups | 5 | Delete in #25 | Exact classes and absence of an inline `top` value are implementation details; Chromium now covers the real height, overflow and responsive behavior. |
